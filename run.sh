...
$EXEC run --rm --name notebook -v $DATA:/data -v $APP:/app -v $TICKTOKEN:/tiktoken --env-file=.env --env "TIKTOKEN_CACHE_DIR=/tiktoken" --network="host" ${IMAGE_PREFIX}trieder83/com-agentic-rag:0.5
$EXEC run --rm --name notebook -v $DATA:/data -v $APP:/app -v $TICKTOKEN:/tiktoken --env-file=.env --env "TIKTOKEN_CACHE_DIR=/tiktoken" --network="host" ${IMAGE_PREFIX}trieder83/com-agentic-rag:0.5
